...
function populateKey() {
	...
	blocksize1 = blocksizeGen(msg.length);
	key1 = keyGen(msg.length / blocksize1);
	// Generate the encrypted message that the user needs to do themselves
	cipher = encryption(msg, key1, blocksize1);
	...
	document.getElementById("blocksize").textContent = blocksize1;
	document.getElementById("msg").textContent = msg;
	
	// Generate a key and initialize key2
	blocksize2 = blocksizeGen(jamesMsg.length);
	key2 = keyGen(jamesMsg.length / blocksize2);
	// Generate the encrypted message that we'll provide for the user
	jamesMsgEncrypted = encryption(jamesMsg, key2, blocksize2);
	...
	document.getElementById("blocksize2").textContent = blocksize2;
